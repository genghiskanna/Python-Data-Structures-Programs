...
int Restore_BST_Property(int TREE[sizeof(a.TREE)],int root,int N)
{
       ...
       a.TREE[root]="NaN"; // Mark the node as deleted
       a.TREE[temp1]=temp;
       if((a.TREE[(2*root)+1]!="NaN")&&((2*root)+1<N)) // A left child exists for the inorder successor
       {
           Restore_BST_Property(a.TREE[sizeof(a.TREE)],(2*root)+1,N);
       }
       if((a.TREE[2*root+2]!="NaN")&&((2*root)+2<N)) // A right child exists for the inorder successor
       {
...
int Delete_Array_BST(int TREE[sizeof(a.TREE)],int N)
{
    ...
    scanf("%d",&element);
    while((a.TREE[temp]!=element)&&(a.TREE[temp]!="NaN")&&(temp<N))
       {
        ...
        if(a.TREE[temp]!="NaN")// If the element is found // Case 1 – Deletion of a leaf node
         {
             if(((2*temp)+2<N)&&((2*temp)+1<N)&&(a.TREE[2*temp+1]=="NaN")&&(a.TREE[(2*temp)+2]=="NaN"))
             {
                //   if() //Node is a leaf node
                //  {
                    a.TREE[temp]="NaN"; //Marked as deleted
                //  }
             }
            ...
            else if((((2*temp)+2)>=N)&&(((2*temp)+1)<N)&&(a.TREE[2*temp+1]=="NaN")) //Node is a leaf node
            {
               // if
                //  {
                    a.TREE[temp]="NaN"; //Marked as deleted
          ...
          {
            if(((2*temp)+1<N)&&(a.TREE[2*temp+1]=="NaN")||((2*temp)+2<N)&&(a.TREE[(2*temp)+2]=="NaN"))
            {
                if(a.TREE[2*temp+1]!="NaN")// The node has left subtree to be managed
                {
            ...
            else  if(((2*temp)+1<N)&&(a.TREE[2*temp+1]!="NaN")&&(2*temp)+2>=N)
            {
                    Restore_BST_Property(a.TREE[sizeof(a.TREE)],(2*temp)+1,N);
            }
            else if((a.TREE[2*temp+1]!="NaN")&&(a.TREE[(2*temp)+2]!="NaN")) // Case 3 – Deletion of a node with both left and right subtrees
            {
                if(((2*temp)+2<N)&&(2*temp+1!="NaN"))
                {
               ...
               else
               {
                ...
                while((2* inorder_successor+1<N)&&(a.TREE[2* inorder_successor+1]!="NaN")) // Locate the inorder successor
                {
                    inorder_successor=2* inorder_successor+1;
                }
                a.TREE[temp]=a.TREE[inorder_successor]; // Replace with inorder successor
                a.TREE[inorder_successor]="NaN";
             if(2* inorder_successor+2<N)
             {
               if(a.TREE[2* inorder_successor+2]=="NaN") // Inorder successor has no child
                {
                    a.TREE[inorder_successor]="NaN";
                }
